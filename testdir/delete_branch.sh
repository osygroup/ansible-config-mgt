...
if [ "$RESULT" ]; then
    ...
    curl -F file=@list.txt -F "initial_comment=Hello Transcode team, kindly prune the following old branches (last commit date in bracket):" -F channels=C0294EZMK9N -H "Authorization: Bearer ${BOT_TOKEN}" https://slack.com/api/files.upload
fi
